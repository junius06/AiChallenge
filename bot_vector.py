...
TRAINING_STATE, TRAINING_QUERY_STATE, ISSUE_STATE, RETRIEVE_STATE, SERVICE_STATE = range(5)
# KEYWORD_STATE, RANDOM_STATE
...
## 데이터가 이미 메모리에 로드되어 있기 때문에 처리 속도가 빠르기 때문에 data.json을 벡터로 임베딩하여 저장하여 랜덤한 문제를 사용한다.
## 유저가 입력하는 키워드와 유사한 값을 가져오기 위해서는 벡터로 임베딩하여 코사인 유사도를 이용하는 것이 유리하다.
class Training:
    def __init__(self, num_issues: int = 3): # chatgpt: ChatGPT,
        self.logger_util = LoggerUtility()
    ...
    async def start_training(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        try:
            ...
            message = "Keyword : 키워드를 기반으로 문제를 제출합니다.\nRandom  : 키워드 관계없이 랜덤한 문제를 제출합니다."
            ...
            await update.message.reply_text(text="오류가 발생했습니다. 다시 시도해주세요.")
            
        return ConversationHandler.END
    
    async def button_query(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        try:
            if query.data == 'cancel':
                ...
                messages = f"{callback_data}을 클릭하여 랜덤한 3개의 문제가 제공됩니다. 문제를 해결하기 위한 방안을 제시하세요.\n'/cancel' 명령으로 트레이닝을 종료할 수 있습니다."
            ...
            self.logger_util.logger.error(f"In 'button_query' funtion : {e}")
    ...
    # 위에 저장된 사용자의 답을 ChatGPT가 예상 해결책의 유사성과 비교하여 평가해주는 함수
...
class Retrieving: # 유사한 키워드도 검색 가능하도록 chatGPT 시켜서 요청하기
    ...
    async def retrieve_cases(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        error_case = update.message.text.lower()
        context.user_data['error_case'] = error_case
        user = self.logger_util.user_info_log(update)
        
        # ChatGPT를 통해 유사 키워드 생성
        try:
            prompt = (
                f"'{error_case}'와 이를 한국어 또는 영어로 번역한 단어를 키워드로 생성합니다."
                f"'{error_case}'와 유사한 단어를 생성하지 않습니다. 언어 무관하나, 무조건 동일한 단어만 생성합니다."
                "'DB'와 같이 축약어의 경우 확장된 단어를 포함하여 생성합니다."
                "당신은 생성한 단어들만 나열하고, 아무런 응답을 하지 않습니다."
                "test1, test2 와 같이 ','를 이용하여 나열합니다."
                "중복되는 단어들은 제거하여 나열합니다." # 대소문자 구분을 확실히 하는 중 - 구분없이 키워드를 검색할 수 있도록 수정이 필요함.
            )
            error_cases = await self.chatgpt.request_chat_gpt(prompt)
            print(f'{error_cases}\n')
            
            # 유사 키워드 목록을 파싱하여 검색
            all_error_cases = re.split(r',|\n', error_cases.strip())
            all_error_cases = [kw.strip().lower() for kw in all_error_cases]
            print(f'{all_error_cases}\n')
            
            matched_cases = []
            for error_case in all_error_cases:
                matched_cases.extend([
                    case for case in self.error_cases if re.search(error_case, case['issue'], re.IGNORECASE)
                ])
            print(matched_cases)
            # 중복 제거
            ...
            log_message = f"The user '{user}' searched for error cases by error_case '{error_case}'."
    ...
    async def service_handle_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):        
        ...
        try:
            if callback_data == 'cancel':
                ...
                prompt = [
                    f"{callback_data_url}페이지를 확인하여 {callback_data} 플랫폼이 무엇인지 주요 특징과 사용 사례에 대해 설명해 주세요."
                    f"{callback_data} 서비스는 블록체인을 기반으로 만들어진 플랫폼입니다. 응답 메세지에는 링크와 '블록체인을 기반으로 한 플랫폼' 문구를 포함하지 않습니다."
                ]
...
async def main() -> None:
    ...
    training_conv_handler = ConversationHandler(
        ...
        states={
            TRAINING_STATE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, training_handler),
                CallbackQueryHandler(training_handler.button_query)
            ]
        },
        fallbacks=[
